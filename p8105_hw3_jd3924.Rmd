...
both happened in Saturday, and almost is the end of the data collection.
```{r}
accel_data %>%
...
```
From the plot above, we can see that during normal bedtime, from 10:00pm to 6:00am, the activity amount is low, which is reasonable.Also, we can see that after a high activity, there always have a rest period after. And the high activity most happens in the morning, and before bedtime. 
During afternoon, the activity is in a normal range. 
And. the day that have the most activity is on Wednesday. 
#Problem 3
```{r}
library(p8105.datasets)
data("ny_noaa")
