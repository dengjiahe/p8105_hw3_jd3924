...
The accelerometer data has 50400 rows and 6 columns. The variable week
tells the which week that the accelerometer data is being collected. The
variable day_id tells which day that the accelerometer data is being
collected. The variable weekday_weekend tells they day is whether
weekday or weekend. The variable activity_min count the activity in
minutes. The variable amount_of_activity tells the amount of activity
have been done.
...
which is 1440. both happened in Saturday, and almost is the end of the
data collection.
...
From the plot above, we can see that during normal bedtime, from 10:00pm
to 6:00am, the activity amount is low, which is reasonable.Also, we can
see that after a high activity, there always have a rest period after.
And the high activity most happens in the morning, and before bedtime.
During afternoon, the activity is in a normal range. And. the day that
have the most activity is on Wednesday. \#Problem 3
...
![](p8105_hw3_jd3924_files/figure-gfm/unnamed-chunk-13-1.png)<!-- -->
